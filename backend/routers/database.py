...
async def get_databases(current_user: dict = Depends(get_current_user)):
    ...
    response = await async_db(
        lambda: supabase.table("databases").select("*").eq("user_id", current_user["id"]).execute()
    )
...
async def create_database(request: CreateDatabaseRequest, current_user: dict = Depends(get_current_user)):
    ...
    # Use async httpx client to avoid blocking the event loop
    ...
    db_response = await async_db(
        lambda: supabase.table("databases").insert(database_data).execute()
    )
